...
export default function OAuthButtons({ disabled = false }) {
  ...
  const providers = site?.oauth_providers?.filter(({ id }) => SUPPORTED_PROVIDERS.has(id)) || [];
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center gap-3 text-xs text-page-secondary">
        <span className="h-px flex-1 bg-page-divider" />
      ...
      <div className="mt-4 grid gap-3">
        {providers.map((provider) => (
          ...
          >
            {loading === provider.id ? (
              <div className="h-4 w-4 rounded-full animate-spin border-2 border-page-divider border-t-current" />
            ) : (
