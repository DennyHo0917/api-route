...
function renderDocsOverview(language) {
  ...
  return [
    ...
    `<section><h2>${escapeHtml(copy.workflow.title)}</h2><h3>${escapeHtml(copy.workflow.stepsTitle)}</h3><ol>${copy.workflow.steps.map(([title, body]) => `<li><strong>${escapeHtml(title)}</strong> ${escapeHtml(body)}</li>`).join('')}</ol><h3>${escapeHtml(copy.workflow.keySettingsTitle)}</h3><p>${escapeHtml(copy.workflow.keySettingsBody)}</p>${definitionList(copy.workflow.keySettingsItems)}${pageLinks(copy.workflow.keySettingsLinks)}<h3>${escapeHtml(copy.workflow.interfaceTitle)}</h3><p>${escapeHtml(copy.workflow.interfaceBody)}</p>${definitionList(copy.workflow.interfaceItems)}<p><a href="${SITE_URL}${localizedPath('/docs/quickstart', language)}">${escapeHtml(copy.workflow.link)}</a></p></section>`,
